...
const UsersComponent = ({...props}: HookData) => {
  ...
  async function onSubmitCreateUser(values: CreateUserGridData) {
    if (values.role == 'owner') {
      await mobxStore.createUser({
        ...values,
        administrator_id: mobxStore.user.id
      }).then(() => mobxStore.getUsers().then(() => setRowData(mobxStore.users)));
      handleReset(values);
  ...
  const deleteUser = useCallback(() => {
    ...
    if (getSelectedNodes) {
      getSelectedNodes.forEach((selectedData) => {
        mobxStore.deleteUser(selectedData.data.id).then(() => mobxStore.getUsers().then(() => setRowData(mobxStore.users)));
      });
    }
  }, []);
  ...
  return mobxStore.user.role !== 'regional_admin' && mobxStore.user.role !== 'admin' ? (
    ...
    (
      <div
        className={mobxStore.user.role == 'regional_admin' || 'admin' ? 'users-grid' : 'users-grid disabled'}
        onClick={() => changeState()}>
        <ModalFunction
          active={createActive}
        ...
        >
          ...
          <div className="buttons-delete">
            ...
            <button className="submit__button-modal submit__button-modal-delete"
              onClick={deleteUser}>
              OK
            </button>
          </div>
                  ...
                  <div className="page-details">
                    <div className="page-details__data">
                      ...
                      <div className="personal-details__information">
                        ...
                        <p
                          className="personal-details__info personal-details__desktop-email">{data.email}</p>
                        <p
                          className="personal-details__info personal-details__mobile-email">{data.email}</p>
        ...
        <div className="grid-function">
          ...
          <div className="grid-buttons">
            ...
            <span className="users-grid__span">
              <img className="users-grid__img" src="icons/material-symbols_delete-outline.svg"
                alt="delete user"/>
            </span>
            </button>
            <button className="users-grid__button" onClick={() => openModal('Edit user')}>
            <span className="users-grid__span">
              <img className="users-grid__img" src="icons/material-symbols_edit-outline.svg"
                alt="edit user"/>
            </span>
            </button>
